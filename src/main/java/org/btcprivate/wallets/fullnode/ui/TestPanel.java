  ...
  public TestPanel(JFrame parentFrame,
    ...
    this.daemonInfoGatheringThread = new DataGatheringThread<>(
        () -> {
          ...
          DaemonInfo daemonInfo = TestPanel.this.installationObserver.getDaemonInfo();
    ...
    ActionListener alDeamonStatus = e -> {
      try {
        TestPanel.this.updateStatusLabels();
      } catch (Exception ex) {
        Log.error("Unexpected error: ", ex);
        TestPanel.this.errorReporter.reportError(ex);
      }
    };
    ...
    this.walletBalanceGatheringThread = new DataGatheringThread<>(
        () -> {
          long start = System.currentTimeMillis();
          WalletBalance balance = TestPanel.this.clientCaller.getWalletInfo();
          long end = System.currentTimeMillis();
          ...
          if (TestPanel.this.walletIsEncrypted == null) {
            TestPanel.this.walletIsEncrypted = TestPanel.this.clientCaller.isWalletEncrypted();
          }
    ...
    ActionListener alWalletBalance = e -> {
      try {
        TestPanel.this.updateWalletStatusLabel();
      } catch (Exception ex) {
        Log.error("Unexpected error: ", ex);
        TestPanel.this.errorReporter.reportError(ex);
      }
    ...
    this.transactionGatheringThread = new DataGatheringThread<>(
        () -> {
          long start = System.currentTimeMillis();
          String[][] data = TestPanel.this.getTransactionsDataFromWallet();
          long end = System.currentTimeMillis();
    ...
    ActionListener alTransactions = e -> {
      try {
        TestPanel.this.updateWalletTransactionsTable();
      } catch (Exception ex) {
        Log.error("Unexpected error: ", ex);
        TestPanel.this.errorReporter.reportError(ex);
      }
    ...
    this.netInfoGatheringThread = new DataGatheringThread<>(
        () -> {
          long start = System.currentTimeMillis();
          NetworkAndBlockchainInfo data = TestPanel.this.clientCaller.getNetworkAndBlockchainInfo();
          long end = System.currentTimeMillis();
